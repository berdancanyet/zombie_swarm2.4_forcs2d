...
--Callback Functions--
function get_item(id,iid) --id, itemid <int, int> 
...
function buy(id,mon,eq,nm) --id, price, equipid, name <int, int, int, string>
	exec("sv_soundpos \"items/pickup.wav\" "..player(id,"x").." "..player(id,"y").."")
	exec("equip "..id.." "..eq) 
	exec("setweapon "..id.." "..eq)
	exec("setmoney "..id.." "..player(id,"money")-mon)
	m2(id,"You have bought a "..nm.." for "..mon.."$@C")
end

function error(id)
	id = tonumber(id)
	m2(id,"You don't have enough money to buy this!@C")
end
